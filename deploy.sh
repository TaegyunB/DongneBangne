...
while [ $MYSQL_RETRY -lt $MAX_MYSQL_RETRIES ] && [ $MYSQL_READY -eq 0 ]; do
    if docker exec $MYSQL_CONTAINER mysqladmin ping -h localhost -u dongnaeuser -pdongnaepass --silent; then
        echo "✅ MySQL 준비 완료"
        MYSQL_READY=1
    else
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    # HTTP 헬스체크 - 여러 엔드포인트 시도
    if curl -f -s http://localhost:8080/actuator/health > /dev/null 2>&1; then
        echo "✅ 서비스 정상 동작 확인! (Actuator 헬스체크 응답)"
        break
    elif curl -s http://localhost:8080 | grep -q "login\|Spring\|<!DOCTYPE\|Whitelabel" > /dev/null 2>&1; then
        ...
        if [ $((RETRY_COUNT % 3)) -eq 2 ]; then
            echo "📊 현재 애플리케이션 상태:"
            docker logs $CONTAINER_NAME --tail 5 || echo "로그 확인 실패"
        fi
